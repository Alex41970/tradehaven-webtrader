...
interface ManagePaymentSettingsDialogProps {
  open: boolean;
...
export const ManagePaymentSettingsDialog: React.FC<ManagePaymentSettingsDialogProps> = ({
  open,
  onOpenChange,
  user,
  onSave
  ...
  useEffect(() => {
    if (open && user?.user_id) {
      fetchPaymentSettings(user.user_id);
    }
  }, [open, user?.user_id, fetchPaymentSettings]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Manage Payment Settings</DialogTitle>
          <DialogDescription>
            Configure payment settings for {userName}
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button onClick={savePaymentSettings} disabled={isSaving || isLoading}>
